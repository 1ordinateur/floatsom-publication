...
\begin{itemize}
\item \textbf{Primary systems contribution:} A distributed, out-of-memory GPU SOM framework that supports multi-GPU execution and disk-backed streaming, scaling SOM training from desktop GPUs to billion-sample workloads across multiple HPC nodes.
...
Sampling comparisons in Section 5.2 compare full versus random paired analyses for the hexagonal topology. Topology comparisons in Sections 5.3-5.4 use full sampling runs across hexagonal, MST, and RNG. Otherwise, the remaining analyses use full sampling. A separate focused HDSSSOM pilot is reported in Section 5.2 and summarized in Supplementary Table S2. This pilot additionally included the HDSSSOM sampling methodology alongside the full and random sampling methodologies.
...
\caption{Hexagonal versus MST topology on $QE$ metrics under full sampling only. Panels A-C report paired full sampling only $QE$ effects for $QE_B$, $QE_H$, and $QE_T$ across the available full sampling datasets. Forest whiskers denote 95\% paired $t$-test confidence intervals around the mean paired effect. Supplementary Table S5 reports the per-dataset effect estimates, 95\% confidence intervals, raw p-values, and dataset-level Benjamini-Hochberg adjusted q-values.}
\label{fig:6}
\end{figure}
...
\caption{Hexagonal versus RNG topology on $QE$ metrics under full sampling only. Panels A-C report paired full sampling only $QE$ effects for $QE_B$, $QE_H$, and $QE_T$ across the available full sampling datasets. Forest whiskers denote 95\% paired $t$-test confidence intervals around the mean paired effect. Supplementary Table S5 reports the per-dataset effect estimates, 95\% confidence intervals, raw p-values, and dataset-level Benjamini-Hochberg adjusted q-values.}
\label{fig:7}
\end{figure}
...
Givanna Putri received access to computational resources from Xenon Systems. Xenon Systems had no influence on the submitted work. The remaining authors declare no competing interests.

\subsubsection*{Funding}

This work was supported by the International Society for the Advancement of Cytometry (ISAC) through the Marylou Ingram Scholars Program awarded to Givanna Putri. This research was supported by the Commonwealth through an Australian Government Research Training Program Scholarship awarded to Tony Xu. Tony Xu also received PhD funding from the Talo Computational Biology Talent Accelerator.

This work was supported by computational resources on Gadi provided by the Australian Government through the National Computational Infrastructure (NCI) under the ANU Merit Allocation Scheme (ANUMAS).
