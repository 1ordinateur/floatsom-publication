...
% Author order, affiliations and equal co-last-author credit restored from
% the preprint source (retained in Git history), using official TMLR macros.
\author{\name Tony Xu \email \href{mailto:tony.xu@anu.edu.au}{tony.xu@anu.edu.au} \\
        \addr John Curtin School of Medical Research, Australian National University
        \AND
        \name Sarah Klamt \email \href{mailto:sklamt@ethz.ch}{sklamt@ethz.ch} \\
        \addr Department of Information Technology and Electrical Engineering, ETH Z\"urich
        \AND
        \name Katharine Turner \email \href{mailto:katharine.turner@anu.edu.au}{katharine.turner@anu.edu.au} \\
        \addr Mathematical Data Science Centre, Australian National University
        \AND
        \name Anne Br\"ustle \email \href{mailto:Anne.Bruestle@anu.edu.au}{Anne.Bruestle@anu.edu.au} \\
        \addr John Curtin School of Medical Research, Australian National University
        \AND
        \name Felix Marsh-Wakefield\thanks{Felix Marsh-Wakefield and Givanna Putri contributed equally as co-last authors.} \email \href{mailto:felix.marsh-wakefield@sydney.edu.au}{felix.marsh-wakef{}ield@sydney.edu.au} \\
        \addr Centenary Institute of Cancer Medicine and Cell Biology\\
        University of Sydney
        \AND
        \name Givanna Putri\footnotemark[1] \email \href{mailto:putri.g@wehi.edu.au}{putri.g@wehi.edu.au} \\
        \addr Walter and Eliza Hall Institute}

\hypersetup{
  pdftitle={FloatSOM: GPU Accelerated, Distributed, Topology-Flexible Self-Organizing Maps},
  pdfauthor={Tony Xu; Sarah Klamt; Katharine Turner; Anne Brüstle; Felix Marsh-Wakefield; Givanna Putri}
}

% Publication metadata confirmed by the corresponding author.
\def\month{09}
\def\year{2026}
\def\openreview{\url{https://openreview.net/forum?id=n2NQNxu9Ei}}

\begin{document}

% Keep spare page height below the text instead of stretching float gaps.
\raggedbottom

\maketitle

\begin{abstract}
GPU-accelerated Self-Organizing Map (SOM) implementations are among the most competitive options for large-scale SOM analysis, but growing dataset sizes increasingly challenge their practical use because workloads no longer fit cleanly within device-memory limits. We introduce FloatSOM, a distributed, GPU-accelerated SOM framework. The primary contribution of this work is a scalable systems architecture supporting multi-GPU execution, out-of-core disk-backed streaming, and hyperparameter tuning. FloatSOM additionally supports scalable training-time graph topologies beyond the regular hexagonal lattice. In our largest benchmark, we trained a 1024-node SOM on a dataset containing 1,000,000,000 samples and 50 features in 6.16 minutes using 8 GPUs across two high-performance computing (HPC) nodes. As a secondary evaluation enabled by this framework, we benchmarked graph-based topologies against the conventional hexagonal topology and found that the former yielded a modest reduction in quantization error.
\end{abstract}

\section{Introduction}

The self-organizing map (SOM), originally introduced by Kohonen \citep{kohonenSelforganizingMap1990}, is an unsupervised machine-learning method that uses competitive learning to organize nodes such that they capture the topology of the data. In practice, this topology-preserving representation means SOMs are commonly used to map dataset topology, produce dimensionality-reduced visualizations, and conduct clustering at scale \citep{kangasVariantsSelforganizingMaps1990}. As dataset size and heterogeneity increase, the computational requirements of SOMs grow in both time and memory. Recent tools such as aweSOM provide optimized single-node serial-online SOM execution for clustering analysis \citep{haAweSOMCPUGPUaccelerated2025}. However, many current implementations remain constrained to single-device workloads that must fit within video random-access memory (VRAM, GPU memory), with limited support for distributed compute, out-of-core execution, and modern GPU orchestration.
...
In this paper, we introduce FloatSOM, a distributed, GPU-accelerated SOM framework that removes these constraints. Our primary research question is whether SOM training can be scaled beyond single-device memory and single-GPU throughput through distributed multi-GPU execution and out-of-core disk-backed streaming. To address this question, FloatSOM implements these systems capabilities together with automated hyperparameter tuning. As a secondary contribution, we also implement scalable minimum-spanning-tree (MST) and relative-neighborhood-graph (RNG) topologies that define the SOM neighborhood during training rather than being derived from a trained map. Accordingly, our primary and secondary contributions are:

\begin{itemize}
\item \textbf{Primary systems contribution:} A distributed, out-of-core GPU SOM framework that supports multi-GPU execution and disk-backed streaming, scaling SOM training from desktop GPUs to billion-sample workloads across multiple HPC nodes.
\item \textbf{Secondary topology contribution:} Scalable training-time graph topologies (MST and RNG), implemented for the first time in a GPU-accelerated, distributed setting.
\end{itemize}
...
Somoclu and GigaSOM provide additional parallel-systems context \citep{wittekSomocluEfficientParallel2017,kratochvilGigaSOMjlHighperformanceClustering2020} and are discussed further in Section 8.

\subsection{Sampling Methodologies for SOM Training}

Classical online and batch SOM training schemes traditionally sample every data point in every training iteration \citep{kohonenSelfOrganizingMaps2001,liuRobustPhenotypingHighly2023}. Consequently, one route to improving SOM speed and scalability is subsampling, which reduces the amount of data used to train the final SOM.
...
If $m\ge N$, the random selector returns the full dataset (no subsampling). Otherwise, to avoid biasing training toward a single fixed subsample, the random selector redraws $\mathcal{I}_t^{\mathrm{random}}$ from the entire dataset at every training iteration. The selected training subset is $X_t^{(s)}=\{x_i:i\in\mathcal{I}_t^{(s)}\}$. Redrawing uniform size-$m$ subsets across iterations improves coverage of the full dataset.
...
\subsection{Multi-GPU and Larger-Than-Memory Methodology and Implementation}

FloatSOM is implemented for distributed GPU execution, with support for RAM (random-access memory, or CPU memory) spilling and disk-backed execution when datasets exceed available VRAM and RAM, respectively.

\begin{figure}[tbp]
\centering
% SVG source: assets/figures/fig_2.svg
\includegraphics[width=0.47\linewidth]{assets/figures/fig_2.pdf}
\caption{Multi-GPU data loading and NCCL synchronization schematic. In disk-backed mode, data are sharded to worker-local storage, loaded chunk-wise into pinned host memory, transferred to GPU with data transfer overlapping computation, processed locally, and synchronized by NCCL all-reduce before one weight update per iteration. In RAM mode, data are instead pre-sharded into worker-local CPU RAM and follow the same pinned-memory-to-GPU path without disk reads.}
\label{fig:2}
\end{figure}
...
The globally synchronized accumulators are:
...
As shown in Fig. 2, each worker uses a chunked loading path from CPU memory to GPU memory. In streaming mode, data are distributed to worker-local disk shards and then read chunk-by-chunk into pinned host memory before transfer to GPU; in RAM mode, data are pre-sharded into worker-local CPU RAM and follow the same pinned-memory path without disk reads. FloatSOM overlaps data transfer and compute across CUDA streams, uses just-in-time (JIT) compilation for the core BMU and update kernels, and synchronizes worker-local accumulators once per iteration with NCCL all-reduce. Weights therefore remain resident on worker GPUs across iterations. For large topologies that would otherwise exceed VRAM, FloatSOM also supports spilling the associated graph-distance or influence structures from VRAM to system RAM.

\subsubsection{Larger-Than-Memory Topology Updates}
...
The full-versus-random sampling comparison in Section 5.2 uses paired hexagonal runs. Topology comparisons in Sections 5.3-5.4 use full sampling runs across hexagonal, MST, and RNG. Otherwise, the remaining analyses use full sampling. A separate focused HDSSSOM pilot is reported in Section 5.2 and summarized in Supplementary Table S2. This pilot included HDSSSOM alongside full and random sampling.
...
Balanced $QE$, denoted $QE_B$, is defined as the mean of $QE_T$ and $QE_H$. $QE_B$ is therefore a composite endpoint that weights fidelity of data representation (train) and generalizability (holdout) equally. Note that in the Optuna runs, $QE_T$ and $QE_H$ are optimized jointly as a two-objective vector, with $QE_B$ only calculated \emph{post hoc}.
...
We recorded train, holdout, and balanced $QE$ together with training time. Within each dataset, paired percentage effects across the 10 seeds were summarized using the Wilcoxon signed-rank location estimate, its exact distribution-free 95\% interval, and a two-sided paired Wilcoxon test; global rows pool the 140 dataset--seed pairs. Supplementary Fig. S1 and Supplementary Table S4 report the hexagonal implementation-calibration results; the detailed MST and RNG comparisons are available in the \href{https://github.com/1ordinateur/floatsom-publication/blob/main/extended_supplementaries.zip}{extended supplementaries}.
...
Additional CPU and RAM resources attached to each GPU are scaled linearly with GPU count while keeping the software environment and benchmark procedure consistent across runs. For scaling benchmarks, each run is terminated if it exceeds 30 minutes of wall-clock time. Per-GPU resource allocation is fixed at one NVIDIA V100 (32 GB VRAM), 12 CPU cores, and 90 GB of system RAM. A full node comprises 4 GPUs and their associated CPU resources, with 400 GB of node-local disk storage.

\subsection{Hyperparameter Tuning and Stability}

To quantify parameter-tuning benefit, we performed an explicit paired analysis between the tuned configuration and the XPySOM untuned default reference. For each sampling mode and topology combination, we first extracted the parameter settings from the best-performing Optuna runs under the benchmark objective for that combination. We then distilled these per-seed best-performing tuned settings into deployable default configurations by taking the mean of numerical parameters and the mode of categorical parameters.
...
For Supplementary Fig. S3, tuned and untuned values were paired within dataset, seed, topology, and split. Supplementary Table S7 pools the 280 matched dataset--seed differences for each prespecified profile--topology--metric contrast and reports the mean paired effect, two-sided one-sample paired $t$-test confidence interval, Cohen's $d_z$, raw p-value, and win/loss/tie counts. These raw p-values are descriptive diagnostics rather than a multiplicity-adjusted family; effect sizes and confidence intervals are the primary summaries. The corresponding dataset-level mean observed metrics are available in the \href{https://github.com/1ordinateur/floatsom-publication/blob/main/extended_supplementaries.zip}{extended supplementaries}. Dataset-level forest-plot results use the convention described above, and overall rows pool all matched pairs with the same paired $t$-test and confidence-interval calculation.

\subsubsection{Hyperparameter stability and dataset-type stratification}

Hyperparameter stability is important because it determines whether a method can be deployed reliably without bespoke retuning. We therefore extracted the top-ranked tuned Optuna trial separately for each seed and compared the recovered hyperparameter values across seeds. We define hyperparameter stability as the variation in these recovered settings. For numerical parameters, stability is measured by the relative difference between runs, $|a-b| / \max(|a|,|b|,\varepsilon)$, where $a$ and $b$ are the values of a given parameter for the two compared seeds and $\varepsilon=10^{-12}$. These relative differences are then averaged across numerical parameters, with lower values indicating higher stability. For categorical parameters, stability is defined as the mismatch rate across the same seed pairs. We report both mean per-parameter stability and an equal-weight overall stability summary across datasets.
...
Under that pilot configuration, HDSSSOM was materially worse than full sampling. In the hexagonal view shown in Fig. 3, full sampling outperformed HDSSSOM in all 50 paired comparisons (10 datasets $\times$ 5 seeds; 50 wins, 0 losses, 0 ties), with dataset-level median balanced-$QE$ improvements ranging from 2.5\% to 209.7\% and a global median improvement of 38.7\%. These HDSSSOM results should therefore be interpreted as a pilot screen under the stated configuration rather than as a comprehensive evaluation of all possible HDSSSOM schedules. Given these large differences, we focused on full and random sampling for the remainder of the manuscript.
...
The full versus random analysis in Fig. 4 was generated from matched hexagonal Optuna runs in which the sampling selector was switched from full to random. Effects were computed within matched seed-specific units and then summarized across seeds. Panels A--C of Fig. 4 summarize the matched full versus random paired effects for balanced, holdout, and train $QE$, respectively, showing that full sampling generally provides equal or better $QE$ than random sampling. Notably, this full versus random separation is much smaller than the full versus HDSSSOM pilot effect shown in Fig. 3; in most dataset-level comparisons, the full versus HDSSSOM improvement is at least twice as large, as reflected in the different axis scales of Figs. 3 and 4.

We find that the effectiveness of random sampling relative to full sampling is scale-dependent. Above $10{,}000$ samples, we detect little paired $QE$ difference, whereas in smaller datasets random sampling shows higher variability and less stable outcomes (Fig. 4D-F). This is consistent with reduced per-iteration sample support under random subsampling (Fig. 4D-F). In this benchmark, the $>10{,}000$ regime is therefore a useful practical proxy for more stable random sampling behavior.
...
Nevertheless, full sampling remains the best sampling strategy for optimal $QE$ results. Accordingly, all remaining analyses reported below rely on full sampling unless otherwise specified.
...
Fig. 6 reports dataset-wise paired improvement summaries (hexagonal over MST), calculated using the statistical procedure in Section 4.4.

Overall, MST outperforms the matched hexagonal baseline on balanced $QE$ (Fig. 6A), indicating a net advantage across train and holdout performance. This aggregate gain is driven more clearly by train $QE$ (Fig. 6C), while holdout $QE$ results are more mixed across datasets (Fig. 6B) and shows no clear overall holdout advantage. The overall paired $t$-test p-values are $p=1.12\times10^{-5}$ for balanced $QE$, $p=0.15$ for holdout $QE$, and $p=0.0064$ for train $QE$. Supplementary Table S5 lists the corresponding per-dataset and overall hexagonal-comparison effect estimates, 95\% confidence intervals, raw p-values, and Benjamini--Hochberg q-values for MST and RNG.

\begin{figure}[tb]
\centering
% SVG source: assets/figures/fig_6.svg
\includegraphics[width=\linewidth]{assets/figures/fig_6.pdf}
\caption{Hexagonal versus MST topology on $QE$ metrics under full sampling. Panels A--C report paired effects for $QE_B$, $QE_H$, and $QE_T$ across the available datasets. Forest whiskers denote 95\% paired $t$-test confidence intervals around the mean paired effect. Supplementary Table S5 reports the per-dataset effect estimates, 95\% confidence intervals, raw p-values, and dataset-level Benjamini--Hochberg adjusted q-values.}
\label{fig:6}
\end{figure}

\subsubsection{RNG}

To evaluate RNG topology performance, we apply the same paired analysis to hexagonal versus RNG, reporting $QE_B$, $QE_H$, and $QE_T$ in Fig. 7.

RNG has lower $QE$ than the matched hexagonal baseline on all reported $QE$ endpoints (Fig. 7A--7C). The overall paired $t$-test p-values are $p=7.4\times10^{-10}$ for balanced $QE$, $p=0.0232$ for holdout $QE$, and $p=4.69\times10^{-6}$ for train $QE$. Supplementary Table S5 lists the corresponding per-dataset and overall hexagonal-comparison effect estimates, 95\% confidence intervals, raw p-values, and Benjamini--Hochberg q-values for MST and RNG.

The main trend in Fig. 7 is that RNG improves on hexagonal most clearly in balanced $QE$ and especially in train $QE$, with the separation most apparent in the real-world and larger datasets where the added flexibility of the graph neighborhood appears more useful than the fixed regular lattice.

\begin{figure}[tb]
\centering
% SVG source: assets/figures/fig_7.svg
\includegraphics[width=\linewidth]{assets/figures/fig_7.pdf}
\caption{Hexagonal versus RNG topology on $QE$ metrics under full sampling. Panels A--C report paired effects for $QE_B$, $QE_H$, and $QE_T$ across the available datasets. Forest whiskers denote 95\% paired $t$-test confidence intervals around the mean paired effect. Supplementary Table S5 reports the per-dataset effect estimates, 95\% confidence intervals, raw p-values, and dataset-level Benjamini--Hochberg adjusted q-values.}
\label{fig:7}
\end{figure}
...
\caption{MST versus RNG topology on $QE$ metrics under full sampling. Panels A--C report paired effects for $QE_B$, $QE_H$, and $QE_T$ across the available datasets. Forest whiskers denote 95\% paired $t$-test confidence intervals around the mean paired effect.}
\label{fig:8}
\end{figure}
...
After examining topology effects while varying radius under a common tuned non-radius configuration, we next quantify the broader effect of complete topology-specific tuning relative to the untuned reference.
...
\caption{Topology runtime comparison at fixed $G=8$ GPUs under full sampling. Panels A-C report mean wall-clock runtime (s) for dimension, sample, and grid size scaling workloads, respectively, with topology traces for hexagonal, MST, and RNG. Error bars denote $\pm 1$ standard deviation across $n=3$ repeated runs per configuration. Detailed numerical summaries are available in the \href{https://github.com/1ordinateur/floatsom-publication/blob/main/extended_supplementaries.zip}{extended supplementaries}.}
\label{fig:14}
\end{figure}
...
Overall, Fig. 15 shows median improvements of 14.5\% in $QE_B$, 9.1\% in $QE_H$, and 22.5\% in $QE_T$ for tuned FloatSOM RNG relative to untuned hexagonal XPySOM, capturing the combined deployment effect of implementation, topology choice, and tuning on $QE$.

The matched untuned hexagonal implementation comparison showed no detectable global balanced-$QE$ gain (median improvement 0.000016\%, $p=0.0923$; Supplementary Table S4); on the common untuned-XPySOM reference scale, tuning accounted for 13.04 percentage points of the full tuned-RNG stack's 14.49\% gain, and topology accounted for the remaining 1.44 points (0.88 for holdout $QE$ and 2.34 for train $QE$; Supplementary Table S6 and the \href{https://github.com/1ordinateur/floatsom-publication/blob/main/extended_supplementaries.zip}{extended supplementaries}), consistent with the direct 1.297\% tuned RNG-over-hexagonal balanced-$QE$ advantage in Supplementary Table S5.
...
In this paper, we introduce FloatSOM, a distributed, out-of-core GPU framework for large-scale SOM training that supports training-time graph topologies. FloatSOM scales SOM training across multiple GPUs and HPC nodes and supports datasets larger than available device memory through disk-backed streaming.
...
The quality experiments use the in-memory pathway, whereas the scaling experiments use Ray. Within the distributed benchmark, scaling depends on workload size: overhead dominates small problems, while larger workloads benefit from parallel throughput and may remain in RAM when the 1-GPU case requires disk backing. Efficiencies above 100\% reflect this change in memory regime rather than superlinear computation.

Regarding existing distributed SOM implementations, Somoclu and GigaSOM are important parallel-systems references. However, we did not benchmark Somoclu because both Somoclu and SomocluGPU were already compared directly with XPySOM in the XPySOM study and found to be more than 10 times slower \citep{manciniXPySomHighPerformanceSelfOrganizing2020}. GigaSOM is a CPU-based algorithm implemented in Julia with perhaps the strongest demonstrated runtime and dataset-size scaling. However, considering GigaSOM's hardware requirements, programming language difference, and dataset availability, a full systems benchmark was outside the scope of this paper. For context, GigaSOM's large-dataset example completed a workflow that trained a 1024-node SOM on 1,167,129,317 events with 18 features in $<25$ minutes on an 11-node, 256-core CPU cluster \citep{kratochvilGigaSOMjlHighperformanceClustering2020}. By comparison, FloatSOM trained a SOM of the same size (1024 nodes) on 1,000,000,000 samples with 50 features, a dataset with approximately $2.4\times$ as many scalar values as GigaSOM's, in 6.16 minutes using 8 GPUs across two HPC nodes.
...
The FloatSOM source code is available at \url{https://github.com/1ordinateur/floatsom}. The manuscript sources, figure assets, benchmark code, and supporting analysis tables are available at \url{https://github.com/1ordinateur/floatsom-publication}. The \href{https://github.com/1ordinateur/floatsom-publication/blob/main/extended_supplementaries.zip}{extended supplementaries ZIP} is available from the same repository.
...
\caption{Full GPU-count scaling context for MST and hexagonal under matched full sampling settings. A-C: MST dimension, sample, and grid-size scaling; D-F: hexagonal dimension, sample, and grid-size scaling. Curves correspond to $G\in\{1,2,3,4,8\}$ GPUs and report mean wall-clock runtime (s) with $\pm 1$ standard-deviation error bars across $n=3$ repeated runs per configuration.}
\label{fig:s4}
\end{figure}
