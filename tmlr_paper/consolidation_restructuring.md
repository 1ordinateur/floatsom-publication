...
The Abstract and Discussion now describe the graph-topology improvement over a tuned hexagonal lattice without presenting topology as the dominant source of improvement. Among the graph topologies, RNG gave the most favorable quantization-error, stability, and node-utilization results, so we retain RNG as the default graph topology. At larger node numbers, where RNG's topology-construction cost increases sharply, we instead suggest the MST topology.
