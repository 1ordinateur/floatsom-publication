...
\TrackedChange{Across the seven tested radii, MST and RNG maintained lower observed $QE_B$ than hexagonal, with adjusted differences evident from $r=1.027$ onward.}

\TrackedChange{Increasing radius eventually exchanged higher $QE_B$ for lower MTR in every topology, but this trade-off emerged earlier and more sharply for hexagonal maps.} \TrackedChange{RNG maintained the lowest observed MTR throughout the sweep, while the node-utilization curves did not indicate that the graph-topology results were driven by poorer use of the available map nodes.} \TrackedChange{The significance symbols in Fig. 9 represent within-topology radius comparisons.}
...
\TrackedChange{$QE$ and MTR capture different properties: $QE$ measures vector-quantization fidelity, while MTR measures local ordering between the first and second BMUs.} \TrackedChange{The fixed QE-tuned profiles are associated with higher MTR in every topology, but the topological cost is far larger for hexagonal maps: balanced MTR is 24.34 tied-rank positions higher than in the untuned profile, compared with 1.43 for MST and 1.11 for RNG.} \TrackedChange{The matched radius sweep reinforces this distinction across the full tested range: MST and RNG maintained lower observed $QE$ than hexagonal, while RNG maintained the lowest observed MTR throughout.} \TrackedChange{Thus, the QE-tuned MST and RNG profiles are associated with far lower ordering penalties than the fixed hexagonal lattice.} \TrackedChange{Topology and hyperparameter choice should therefore be considered jointly rather than treating topology as a post-training visualization choice.} \TrackedChange{The lower seed-to-seed variability of MST and RNG may likewise reflect their ability to rebuild connectivity from evolving node locations, unlike the fixed hexagonal lattice \citep{kohonenEssentialsSelforganizingMap2013,kangasVariantsSelforganizingMaps1990}.}
