...
\TrackedChange{Relative to default aweSOM, untuned hexagonal FloatSOM reduced quantization error across all five evaluated datasets.} \TrackedChange{Averaged equally across datasets, the improvements were 36.77\% for balanced $QE$, 28.52\% for holdout $QE$, and 45.11\% for train $QE$.} \TrackedChange{The complete dataset-level results are reported in Supplementary Table S20.} \TrackedChange{Given the large difference between aweSOM's serial-online training and the batch training used by FloatSOM and XPySOM, together with the large-workload timeout documented in the speed-benchmark protocol (Section 4.2), we used XPySOM as the external implementation comparator in subsequent analyses.}
...
\TrackedChange{Separation vanished at $r\leq0.75$, with no significant MST--hexagonal or RNG--hexagonal difference at either $r=0.5$ or $r=0.75$ (Supplementary Table S15).} \TrackedChange{The per-topology minimum $QE_B$ across the sweep occurred at $r=0.75$ for hexagonal and $r=1.5$ for MST and RNG.} \TrackedChange{In paired tests between those minima, MST was 0.98\% lower than hexagonal (ratio 0.9902, 95\% CI [0.9747, 1.0060]; $p=0.203$, $q=0.304$), and RNG was 1.35\% lower than hexagonal (ratio 0.9865, 95\% CI [0.9689, 1.0043]; $p=0.125$, $q=0.304$); neither difference was significant.} \TrackedChange{RNG and MST also did not differ significantly (ratio 0.9962, 95\% CI [0.9834, 1.0092]; $p=q=0.538$; Supplementary Table S16).}

\TrackedChange{The split-specific aggregate comparisons followed the same pattern.} \TrackedChange{Relative to hexagonal, MST had 0.69\% lower $QE_H$ ($p=0.316$, $q=0.474$) and 2.19\% lower $QE_T$ ($p=0.145$, $q=0.257$), while RNG had 0.79\% lower $QE_H$ ($p=0.139$, $q=0.418$) and 4.04\% lower $QE_T$ ($p=0.171$, $q=0.257$).} \TrackedChange{RNG was 0.10\% lower than MST for $QE_H$ ($p=q=0.773$) and 1.90\% lower for $QE_T$ ($p=q=0.332$).} \TrackedChange{Thus, all split-specific aggregate point estimates favoured the graph comparator, but none was significant (Supplementary Table S16).}

\TrackedChange{These aggregate summaries should be interpreted alongside substantial dataset-level heterogeneity in both direction and magnitude.} \TrackedChange{RNG had lower observed $QE_H$, $QE_T$, and $QE_B$ than hexagonal in 12, 8, and 9 of the 14 datasets, respectively, with significant improvements in six, seven, and four datasets and significant reversals in one, five, and one datasets.} \TrackedChange{MST had lower observed $QE_H$, $QE_T$, and $QE_B$ in 9, 8, and 9 datasets, with significant improvements in four, three, and four datasets and significant reversals in one, three, and one datasets (Supplementary Tables S17--S18).}

\TrackedChange{Moreover, where graph topologies were significantly better, the relative improvements reached 15.44\% for MST and 26.49\% for RNG, substantially larger than the largest significant hexagonal advantage of 2.23\%.} \TrackedChange{For example, RNG reduced Iris $QE_H$, $QE_T$, and $QE_B$ by 2.26\%, 26.49\%, and 7.28\%, respectively, with all three differences significant.}

\TrackedChange{Increasing radius eventually exchanged higher $QE_B$ for lower observed MTR in every topology, but this trade-off emerged earlier and more sharply for hexagonal maps.} \TrackedChange{RNG maintained the lowest observed MTR throughout the sweep, while the node-utilization curves did not indicate poorer use of the available map nodes.} \TrackedChange{Because these raw MTR curves compare a fixed lattice with weight-derived graphs, their cross-family separation is descriptive.} \TrackedChange{The significance symbols in Fig. 9 represent within-topology radius comparisons.}
