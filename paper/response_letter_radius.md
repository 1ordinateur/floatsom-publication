...
The matched diagnostics show that the RNG improvement is not strictly a tuning artifact. Under the untuned/default profile, RNG outperformed untuned/default hexagonal maps on balanced MTR by 2.48 tied-rank positions (95% CI 2.22 to 2.75; p=2.44e-45; 200/210 matched pairs favoring RNG), and also on balanced $QE$ (effect favoring RNG=0.249; 95% CI 0.172 to 0.327; p=1.63e-9). Under the tuned profile, RNG had a larger balanced MTR advantage of 25.58 tied-rank positions (95% CI 24.53 to 26.63; p=4.10e-115; 210/210 matched pairs favoring RNG).

We also added the tuned-versus-default MTR comparison because it directly addresses whether $QE$ optimization itself changes topology preservation. Tuning improved balanced $QE$ for hexagonal, MST, and RNG, but worsened balanced MTR in each topology. The MTR penalty was much larger for hexagonal (24.36 tied-rank positions) than for MST (1.41) or RNG (1.26). We interpret this as evidence that the fixed hexagonal lattice reaches lower optimized $QE$ at a much larger cost to local BMU-neighborhood ordering than the graph topologies do.

Node utilization results support this interpretation, as the reviewer kindly pointed out. Considering this, we calculated node utilization and dead-node fraction for the same matched maps and showed that the MTR results are not explained by graph topologies abandoning map units. For example, tuned RNG increased balanced node utilization by 0.0207 (95% CI 0.0163 to 0.0250; p=1.52e-17) and reduced balanced dead-node fraction by the same amount. We embedded the full paired diagnostic table and full by-dataset diagnostic table in the manuscript supplement rather than leaving them as external TSV-only outputs.
...
The matched tuned-profile result favored RNG rather than indicating poorer map use. Relative to tuned hexagonal maps, tuned RNG increased balanced node utilization by 0.0207 (95% CI 0.0163 to 0.0250; p=1.52e-17) and reduced balanced dead-node fraction by the same amount. Holdout-only node utilization was also positive (0.0140; 95% CI 0.00882 to 0.0191; p=2.18e-7), so we report the split-specific and balanced diagnostics separately and do not present node utilization as the primary topology-preservation statistic.
