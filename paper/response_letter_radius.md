...
GigaSOM.jl is also important, especially for large cytometry workloads. We added manuscript text noting that its IMPC demonstration trained a $32 \times 32$ SOM on 1,167,129,317 cells and completed the full Julia workflow, including data loading, 30 SOM epochs, embedding, and export, in under 25 minutes on an 11-node, 256-core CPU cluster [@kratochvilGigaSOMjlHighperformanceClustering2020]. This is an important systems precedent, but not a direct executable baseline for our Python/CUDA/Ray setting because the implementation language, hardware, feature dimensionality, epoch count, and included workflow stages differ. We therefore discuss GigaSOM as a distributed large-scale SOM system precedent rather than as a directly benchmarked external Python implementation.
