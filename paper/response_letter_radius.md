...
The matched tuned-profile result favored RNG rather than indicating poorer map use. Relative to tuned hexagonal maps, tuned RNG increased balanced node utilization by TBD (95% CI TBD to TBD; p=TBD) and reduced balanced dead-node fraction by the same amount. Holdout-only node utilization was also positive (TBD; 95% CI TBD to TBD; p=TBD), so we report the split-specific and balanced diagnostics separately and do not present node utilization as the primary topology-preservation statistic.
