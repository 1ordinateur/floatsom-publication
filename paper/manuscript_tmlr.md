...
Globally, both MST and RNG outperform the fixed hexagonal topology in these comparisons, while the two graph topologies remain close competitors on $QE$. A geometric interpretation consistent with the diagnostics begins with neighborhood coupling. A fixed lattice imposes predetermined neighbor identities independently of the learned data-space geometry, so prototype movement can influence lattice neighbors that are locally unrelated, potentially displacing useful prototypes and increasing dead nodes. MST minimizes this coupling and gives prototypes greater freedom to redistribute along irregular or elongated structures, but its tree constraint cannot retain multiple locally appropriate connections where a concentrated region is better represented by a mesh. RNG is less free than MST because nodes may influence several neighbors; those additional connections, however, arise from the evolving prototype geometry rather than being imposed before training. RNG can therefore remain sparse where appropriate and form multiple connections in dense regions. The $QE$, MTR, node-utilization, and dead-node results are consistent with this interpretation, but do not identify a unique causal mechanism [@kohonenEssentialsSelforganizingMap2013; @kangasVariantsSelforganizingMaps1990; @toussaintRelativeNeighbourhoodGraph1980].
