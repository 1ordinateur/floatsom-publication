...
The available staging diagnostics show that disk-backed execution is pushed to larger workloads as GPU count increases: in sample scaling, it shifts from 2 GPUs at 500,000,000 samples to 8 GPUs at 1,000,000,000 samples.
...
This links the Fig. 9 random-versus-full result to the multi-GPU/OOM scaling result on the same 1,000,000,000-sample RNG workload: at 4 GPUs, moving from random to full adds 369.48 s (6.16 min) (+63.41%), but the 8-GPU full configuration finishes that same workload 61.20% faster than the 4-GPU full run and 36.60% faster than the 4-GPU random run.
