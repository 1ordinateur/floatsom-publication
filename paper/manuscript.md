...
FloatSOM is a suite for graphics processing units (GPUs) and Self-Organizing Maps (SOMs), designed for large-scale training and deployment. The framework combines algorithmic optimization, sampling-strategy analysis, flexible topology support (hexagonal, minimum-spanning-tree (MST), and relative-neighbourhood graph (RNG)), and hyperparameter optimization. It supports multi-GPU execution and out-of-memory training through disk-backed streaming while extending SOM topology choices beyond regular lattices. We also evaluate when reduced-sampling regimes are practically appropriate. Taken together, these results define operating regimes for SOM deployment under different quality-throughput constraints. In our largest benchmark, FloatSOM trains a 1024-node network on 1 billion samples with 50 features in 6.16 minutes.

## 1. Introduction

Self-Organizing Maps (SOMs), originally introduced by Kohonen [@kohonenSelforganizingMap1990], learn a neighborhood-preserving map of prototype-node interactions that provides a topology-aware low-dimensional representation of high-dimensional data. In practice, SOMs are commonly used to map dataset topology, produce dimensionality-reduced representations, and support downstream clustering at scale. As dataset size and heterogeneity increase, these use cases become increasingly sensitive to training throughput and memory scaling. Graphics processing units (GPUs) are therefore central to modern SOM deployment, yet many implementations remain constrained to single-device workloads that fit entirely within video random-access memory (VRAM) and fixed lattice neighborhoods (most commonly hexagonal or rectangular grids), with limited support for multi-GPU/out-of-core execution, flexible graph topologies, and principled hyperparameter optimization. This manuscript examines four related areas in that setting: sampling strategy, topology flexibility beyond fixed lattices, multi-GPU and out-of-memory (OOM)-capable execution beyond single-device VRAM limits, and data-backed hyperparameter optimization. Because sampling strategy directly affects the practical quality-throughput tradeoff, we evaluate full, random, and hierarchical dynamic subset selection SOM (HDSSSOM) sampling under matched optimization settings, with detailed sampling methodology and literature context provided in Section 2.1.

At the systems level, we implement a multi-GPU, multi-node training method using Ray actor orchestration with NVIDIA Collective Communications Library (NCCL) collectives, enabling direct GPU-to-GPU communication in environments that support this path. We also implement an OOM-resilient disk-streaming data path through a custom loader for data staging and transfer between storage and GPU memory. In this setting, trainable SOM workload size is shaped primarily by available storage and input/output (I/O) throughput rather than aggregate VRAM alone. Although this execution stack is tuned for high-performance computing (HPC) operation, we also report runs on consumer-grade desktop GPUs in non-HPC environments.

Our experimental program centers on five outcomes. Quality comparisons use quantization error ($QE$) as the primary endpoint because distortion is not defined for MST in this setup:
...
Related work on practical SOM deployment spans four connected threads: sampling-efficient training, open-source implementations and systems acceleration, topology design beyond regular lattices, and model selection/tuning. FloatSOM sits at the intersection of those threads rather than inside only one of them, so we survey each briefly before positioning the present contribution.

### 2.1 Sampling Methodologies for SOM Training

Sampling strategies in SOM pipelines are primarily motivated by computational efficiency. Classical SOM training already spans a spectrum from full-data passes to stochastic presentation of randomly ordered observations, and Kohonen's standard treatments argue that exact exposure to every sample at every iteration is often unnecessary for useful maps [@kohonenSelforganizingMap1990; @kohonenSelfOrganizingMaps2001]. In large modern datasets, the practical objective is therefore not simply to use fewer observations, but to do so without materially changing the map one would have obtained under a larger training budget. This sampling question is distinct from the separate online-versus-batch training choice: a SOM can use full or random sampling under either update regime, whereas online and batch refer to how updates are accumulated and applied.

Within that acceleration framing, random sampling in SOM pipelines appears in two common schemas. In the fixed-subset schema, a random subset is sampled once up front and reused throughout training; this is the strategy used in scalable pixel-level clustering for a biological application and relies on the assumption that the selected subset remains representative of the full dataset [@liuRobustPhenotypingHighly2023]. In the iteration-resampled schema, each training iteration receives a new random draw from the full dataset, using probabilistic coverage across iterations to approximate full-data exposure; this second mode is explicitly discussed in Kohonen's standard SOM treatment as often sufficient in practical settings [@kohonenSelfOrganizingMaps2001]. The fixed-subset variant reduces repeated data access, whereas the iteration-resampled variant reduces the risk that one unlucky draw dominates the whole run.

Beyond naive random sampling, additional methods aim to improve the quality-time tradeoff through guided subset selection. A prominent example is hierarchical dynamic subset selection SOM (HDSSSOM), which ranks regions and exemplars by age and difficulty so the training budget is concentrated on hard or stale parts of the dataset [@wetmoreSpeedingSelfOrganizingFeature2005]. This places HDSSSOM closer to active or curriculum-style selection than to simple subsampling. However, such methods are much less common in maintained high-performance toolchains than straightforward random or full-data training, which is one reason they remain under-compared in contemporary GPU-oriented SOM studies.

### 2.2 Open-Source SOM Implementations and Systems

Open-source SOM software spans lightweight educational libraries and performance-oriented systems. MiniSom has been especially influential at the interface level because it offers a compact, readable Python API that many users encounter first [@vettigliJustGlowingMinisom2026]. Its influence is therefore partly ecosystem-level: it lowered the barrier to experimentation and shaped expectations for Python SOM workflows, even though it is not intended as a large-scale performance baseline.

At the performance-oriented end, prior systems take different implementation paths. XPySOM is a high-performance Python implementation of batch SOM with NumPy/CuPy backends and optimized single-process execution on CPUs or GPUs [@manciniXPySomHighPerformanceSelfOrganizing2020]. Somoclu instead emphasizes massively parallel training through a C++ core with OpenMP, MPI, and CUDA support, exposing interfaces to Python, R, and MATLAB and targeting large emergent maps [@wittekSomocluEfficientParallel2017]. Taken together, these tools show that SOM acceleration is not new; what differs across implementations is the combination of language ecosystem, execution model, topology support, and memory model. They also suggest a broader practical pattern: the implementations that reach the largest workloads are predominantly batch-based rather than classical online/sample-wise trainers, because batch updates align more naturally with vectorized linear algebra, GPU kernels, and chunked memory management.
...
Classical SOM practice overwhelmingly uses regular lattice neighborhoods because they make neighborhood distance, visualization, and vectorized updates straightforward [@kohonenSelforganizingMap1990; @kohonenEssentialsSelforganizingMap2013]. We support both grid and hexagonal regular lattices; however, following prior SOM literature, we treat hexagonal as the standard regular-topology baseline because its neighborhood geometry is typically more isotropic and tends to reduce directional lattice bias relative to rectangular grids [@kohonenEssentialsSelforganizingMap2013; @forestSurveyImplementationPerformance2020a].

Not all work on SOM neighborhoods changes the graph itself. Some studies keep the regular lattice but modify the neighborhood function or training dynamics. For example, Aoki and Aoyagi proposed asymmetric neighborhood functions to speed ordering while managing the distortion introduced by that asymmetry [@aokiSelforganizingMapsAsymmetric2007]. This line of work is important because it shows that the literature has long treated neighborhood design as an optimization lever, even when the underlying lattice remains fixed.

A second topology-flexibility line uses adaptive map growth or movement rather than a fixed graph family. Growing variants address the limitation of pre-specifying map size by inserting neurons during training, and directed batch growing SOM (DBGSOM) reports stronger topology preservation and clustering than GSOM and conventional SOM on several benchmark datasets [@vasighiDirectedBatchGrowing2017]. Adaptive Moving SOM (AMSOM) goes further by allowing neuron addition, removal, and movement during training, and reports improved training performance and visualization on multiple literature datasets while explicitly noting that growing variants increase computational cost [@spanakisAMSOMAdaptiveMoving2016]. These results are important evidence that explicit topology adaptation can work well. However, they also illustrate a systems tradeoff: repeated growth, relocation, and pruning of the map are difficult to reconcile with the high-throughput regime targeted here, and for multi-GPU plus OOM-safe training at the scales studied in this manuscript that degree of dynamic topology maintenance would likely be speed-prohibitive relative to fixed-size graph topologies.

Graph-structured neighborhoods have also been discussed for decades, but more sporadically. Kangas, Kohonen, and colleagues proposed defining SOM neighborhoods via a minimum spanning tree and argued that tree-based neighborhoods could better approximate strongly structured densities than fixed lattices [@kangasVariantsSelforganizingMaps1989; @kangasVariantsSelforganizingMaps1990]. Jang et al. later used minimal spanning trees together with SOM analysis and map-structure selection on smaller examples [@jangUseMinimalSpanning2009]. These works establish that MST-style thinking is not new in SOM research, but they do not resolve the systems question of how to make such topologies practical inside a modern high-throughput training loop.

We also study proximity-graph neighborhoods using Relative Neighborhood Graphs (RNGs) [@toussaintRelativeNeighbourhoodGraph1980]. In the standard RNG construction, two prototypes are connected when no third prototype lies closer to both endpoints than the endpoints are to each other under the blocker criterion. This yields a geometry-driven sparse graph that can preserve local neighborhood relations without imposing a tree constraint. RNG is therefore a natural comparator for MST: it remains sparse and geometry-driven while allowing more local routes than a tree, giving an intermediate point between rigid lattices and strictly tree-structured neighborhoods.

The broader systems gap is that irregular graphs are harder to construct, refresh, and query efficiently during training than fixed lattices. As a result, the literature contains proposals for non-lattice neighborhoods and, separately, high-performance SOM systems, but much less work that joins topology flexibility to distributed GPU execution and out-of-core data movement in a single openly usable workflow. That junction is the specific gap targeted in this manuscript.

### 2.4 XPySOM Compared to FloatSOM 

Among prior open-source systems, XPySOM is the closest algorithmic comparator for our calibration experiments because it shares the core batch-update structure and a Python/CuPy execution model [@manciniXPySomHighPerformanceSelfOrganizing2020]. FloatSOM and XPySOM therefore overlap on the underlying batch-SOM mathematics, which makes their comparison useful for checking that a larger systems stack does not change the expected batch behavior.

The XPySOM implementation follows a single-process NumPy/CuPy design with local chunk-level parallelism. Its batch update forms per-sample neighborhood weights and applies normalized updates with zero-denominator protection. By design, it does not include distributed collectives, multi-GPU actor orchestration, or out-of-core data loading. In contrast, our batch pipeline builds on this optimized batch-SOM foundation while adding just-in-time (JIT)-compiled GPU-kernel paths plus bounded-memory chunked execution for OOM-resilient operation, and is embedded in a Ray+NCCL distributed runtime with explicit multi-GPU synchronization and topology-aware extensions (MST/RNG).

Accordingly, the comparison used here is not intended to claim that earlier systems lacked acceleration. Rather, it isolates the additional execution envelope targeted by FloatSOM: multi-GPU worker groups, disk-backed loading, bounded-memory chunking, and train-time irregular topologies. XPySOM is treated as the closest Python batch-SOM reference in the calibration analyses, while systems such as Somoclu provide broader context that large-scale SOM execution has long been an active systems concern [@wittekSomocluEfficientParallel2017].

### 2.5 Hyperparameter Optimization with Tree-structured Parzen Estimator (TPE)

A separate strand of related work concerns model selection. SOM quality is sensitive to choices such as map size, neighborhood schedule, learning-rate schedule, and initialization, and prior work shows that even the relative value of principal-component versus random initialization depends on data geometry [@akindukoSOMStochasticInitialization2016a]. Survey work on SOM performance metrics likewise emphasizes that model assessment is multi-faceted: quantization accuracy, topographic preservation, and task-specific usefulness do not always move together [@forestSurveyImplementationPerformance2020a]. This makes implementation comparisons based only on untuned defaults difficult to interpret.

More general machine-learning literature has therefore increasingly treated hyperparameter search as an optimization problem in its own right [@bergstraAlgorithmsHyperParameterOptimization2011]. Optuna operationalizes this perspective through flexible Tree-structured Parzen Estimator (TPE)-based search [@akibaOptunaNextgenerationHyperparameter2019a]. In this work, we use TPE-driven optimization to derive hyperparameter sets for each FloatSOM configuration. The effective optimum depends on the selected topology, sampling regime, and processing mode, so tuning is treated as part of the evaluation pipeline rather than as a fixed post hoc setting. This methodology also allows examination of the best performance each configuration can attain under a common search budget, helping disentangle architectural advantages from mere hyperparameter appropriateness.
...
As shown in Fig. 1, each worker uses a chunked host-to-device staging pipeline coupled to distributed iteration control. In streaming mode, data are distributed to worker-local disk shards and then read chunk-by-chunk into pinned host memory before transfer to GPU. In RAM mode, data are pre-sharded directly into each worker's GPU-local CPU RAM and fed into the same pinned-memory staging path, bypassing disk reading. In both cases, the worker processes its assigned shard as `n_chunks` rather than materializing the full shard on device.

The loader maintains bounded in-memory queueing and fetches the next chunk scheduled after the current one. CUDA events coordinate the multi-buffer pipeline so that, in steady state, one chunk can be under GPU compute while the next chunk is being transferred and another host buffer is being prepared. For each chunk, the worker performs BMU search and accumulates local update and influence tensors.

After all `n_chunks` for the current iteration have been processed on each worker, NCCL performs a synchronous SUM all-reduce over the worker-local accumulators, with dtype and contiguity normalization used to keep collective signatures identical across ranks. The synchronized tensors are then normalized and applied once per iteration. Weights remain resident on worker GPUs across iterations, and the driver exchanges lightweight metadata rather than full weight tensors except when an explicit topology refresh fetch is required.
