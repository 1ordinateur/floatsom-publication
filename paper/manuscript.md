...
Self-Organizing Maps (SOMs), originally introduced by Kohonen [@kohonenSelforganizingMap1990], are an unsupervised machine learning technique that uses competitive learning to organize nodes such that they capture the topology of the data. In practice, this topology-preserving representation means SOMs are commonly used to map dataset topology, produce dimensionality-reduced visualizations, and conduct clustering at scale [@kangasVariantsSelforganizingMaps1990]. As dataset size and heterogeneity increase, however, the computational requirements of SOMs grow in both time and memory. In modern accelerated pipelines, this has favored batch-style training, where each iteration evaluates the presented samples against the map, accumulates neighborhood-weighted update statistics across that set, and then applies a coordinated prototype update. In most practical settings this BatchSOM regime has become the dominant approach because it offers superior stability, speed, and output quality relative to online sample-wise updates, while also aligning naturally with vectorized linear algebra and GPU execution [@kohonenEssentialsSelforganizingMap2013; @manciniXPySomHighPerformanceSelfOrganizing2020]. By contrast, MiniSom [@vettigliJustGlowingMinisom2026] exemplifies the lightweight online paradigm, where updates are applied incrementally one sample at a time rather than being accumulated across the dataset before an update step. Even among batch-oriented implementations such as XPySOM [@manciniXPySomHighPerformanceSelfOrganizing2020], many current systems remain constrained to single-device workloads that must fit within video random-access memory (VRAM), with limited support for distributed compute, out-of-core execution, and modern GPU orchestration.

A second practical limitation concerns topology. Classical SOMs are usually trained on regular rectangular or hexagonal lattices because fixed grids make neighborhood definition, visualization, and optimization straightforward. However, these regular lattices also impose a strong geometric prior on the learned representation. Rectangular grids are especially restrictive, and even hexagonal lattices, while often preferred because they reduce directional bias, still assume that the data are best represented by a fixed mesh structure [@kohonenEssentialsSelforganizingMap2013; @forestSurveyImplementationPerformance2020a]. Prior work on dynamic, growing, and graph-structured SOM variants reflects a long-standing recognition that fixed lattices are not always the best match for irregular data geometry [@alahakoonDynamicSelforganizingMaps2000; @vasighiDirectedBatchGrowing2017; @kangasVariantsSelforganizingMaps1990]. However, these alternatives have generally not been developed or evaluated in the high-throughput, large-sample regime targeted by modern GPU-enabled applications, and are broadly impractical for deployment at scale.

FloatSOM is designed to address these combined systems and topology limitations. Within FloatSOM, we implement distributed multi-GPU execution, out-of-memory disk-backed streaming, and scalable topology-flexible training beyond standard fixed lattices. We also evaluate multiple sampling strategies as a route to further computational acceleration, and derive fine-tuned hyperparameter configurations across diverse datasets and recommended operating regimes. FloatSOM is suitable for both high-performance computing (HPC) operation, and also operation on consumer-grade desktop GPUs.

## 2. Related Work

Related work on practical SOM deployment spans software implementations, sampling-efficient training, topology design, and model selection. Because this manuscript is concerned with the practical operating envelope of SOMs rather than a single isolated algorithmic novelty, we review those strands together and focus on what is openly available, what remains limited, and where the main gaps persist.

### 2.1 Open-Source SOM Implementations and Systems

Open-source SOM software ranges from lightweight educational libraries to performance-oriented systems. MiniSom has been especially influential because its compact Python API lowered the barrier to experimentation and helped shape expectations for Python SOM workflows [@vettigliJustGlowingMinisom2026]. At the performance-oriented end, XPySOM provides a high-performance Python batch-SOM implementation with NumPy/CuPy backends [@manciniXPySomHighPerformanceSelfOrganizing2020], while Somoclu emphasizes large-scale parallel execution through a C++ core with OpenMP, MPI, and CUDA support [@wittekSomocluEfficientParallel2017].

Taken together, these tools show that SOM acceleration is not new. However, they also reveal a recurring split in the available software landscape: accessible Python packages often prioritize simplicity, whereas high-performance systems typically prioritize fixed regular lattices, batch-oriented execution, and workloads that remain within the assumptions of a single process or a more narrowly defined systems model. Openly maintained tools that combine Python usability with both topology flexibility and a broader large-scale execution envelope remain comparatively scarce.

### 2.2 Sampling Methodologies for SOM Training

Sampling strategies in SOM pipelines are primarily motivated by computational efficiency. Classical SOM training already spans a spectrum from full-data passes to stochastic presentation of randomly ordered observations, and Kohonen's standard treatments argue that exact exposure to every sample at every iteration is often unnecessary for useful maps [@kohonenSelforganizingMap1990; @kohonenSelfOrganizingMaps2001]. This sampling question is separate from the online-versus-batch distinction: a SOM can use full or sampled data under either update regime, whereas online and batch refer to how updates are accumulated and applied.

Within that framing, random sampling usually appears in two forms. In a fixed-subset schema, one subset is drawn and reused through training; in an iteration-resampled schema, each iteration receives a fresh draw from the full dataset [@kohonenSelfOrganizingMaps2001; @liuRobustPhenotypingHighly2023]. Beyond naive random sampling, guided subset-selection methods such as hierarchical dynamic subset selection SOM (HDSSSOM) attempt to concentrate computation on difficult or stale regions of the data [@wetmoreSpeedingSelfOrganizingFeature2005]. The literature therefore offers several plausible sampling strategies, but systematically benchmarked and openly maintained implementations that compare full-data, random, and guided sampling within the same modern high-performance SOM workflow remain limited.

### 2.3 Regular Grids and Alternative Topologies

Most practical SOM implementations retain regular rectangular or hexagonal lattices because they simplify neighborhood indexing, visualization, and vectorized updates [@kohonenSelforganizingMap1990; @kohonenEssentialsSelforganizingMap2013]. Hexagonal grids are often preferred in the literature because their neighborhood geometry is more isotropic and tends to reduce directional bias relative to rectangular grids [@kohonenEssentialsSelforganizingMap2013; @forestSurveyImplementationPerformance2020a]. More broadly, the dominance of regular grids in software is as much a systems convenience as it is an algorithmic preference.

At the same time, the SOM literature has long explored alternatives to fixed lattices. Some work modifies neighborhood functions while retaining the lattice [@aokiSelforganizingMapsAsymmetric2007]. Other work allows the map to grow, move, or adapt during training, as in DBGSOM and AMSOM [@vasighiDirectedBatchGrowing2017; @spanakisAMSOMAdaptiveMoving2016]. Graph-structured neighborhoods have also been proposed, including minimum spanning tree formulations in early SOM work [@kangasVariantsSelforganizingMaps1989; @kangasVariantsSelforganizingMaps1990] and later smaller-scale MST-based analyses [@jangUseMinimalSpanning2009]. Relative Neighborhood Graphs (RNGs) provide another geometry-driven sparse graph family that is well established in computational geometry [@toussaintRelativeNeighbourhoodGraph1980], but has seen little adoption in openly available SOM toolchains.
...
Model selection is a separate but closely related issue. SOM performance depends strongly on choices such as map size, initialization, learning-rate schedule, and neighborhood schedule, and prior work shows that these choices can materially affect observed performance and even the apparent advantage of one variant over another [@akindukoSOMStochasticInitialization2016a; @forestSurveyImplementationPerformance2020a]. This makes comparisons based only on untuned defaults difficult to interpret.

More generally, machine-learning practice increasingly treats hyperparameter search as part of the comparison problem itself [@bergstraAlgorithmsHyperParameterOptimization2011]. Tools such as Optuna provide practical Tree-structured Parzen Estimator (TPE)-based search for this purpose [@akibaOptunaNextgenerationHyperparameter2019a]. In the SOM setting, this matters because the effective optimum may depend on topology, sampling regime, and update mode, so fair comparison requires more than simply running each implementation with one default configuration.
