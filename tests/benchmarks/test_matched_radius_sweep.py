...
def radius_runner_module():
    ...
    previous = {name: module for name, module in sys.modules.items() if name == "floatsom_benchmarks" or name.startswith("floatsom_benchmarks.")}
    for module_name in list(previous):
        sys.modules.pop(module_name, None)
    package = types.ModuleType("floatsom_benchmarks")
    package.__path__ = [str(_REPO_ROOT / "benchmarks")]
    sys.modules["floatsom_benchmarks"] = package
    ...
    for module_name in list(sys.modules):
        if module_name == "floatsom_benchmarks" or module_name.startswith("floatsom_benchmarks."):
            sys.modules.pop(module_name, None)
