...
\author{\textbf{Tony Xu}\\
{\small\textit{John Curtin School of Medical Research, Australian National University}}\\
{\small\texttt{tony.xu@anu.edu.au}}\\[1em]
\textbf{Sarah Klamt}\\
{\small\textit{Department of Information Technology and Electrical Engineering, ETH Zurich}}\\[1em]
\textbf{Katherine Turner}\\
{\small\textit{Mathematical Data Science Centre, Australian National University}}\\[1em]
\textbf{Anne Bruestle}\\
{\small\textit{John Curtin School of Medical Research, Australian National University}}\\[1em]
\textbf{Givanna Putri}\\
{\small\textit{Walter and Eliza Hall Institute}}\\[1em]
\textbf{Felix Marsh-Wakefield}\\
{\small\textit{Centenary Institute of Cancer Medicine and Cell Biology}}\\
{\small\textit{University of Sydney}}}
\date{}
...
\begin{algorithmic}[1]
\State Input: node weights W\_t, iteration t, topology-refresh policy
\State Output: topology state (E\_t, g\_t, cached influences)
\State Query the refresh policy for the current iteration
\State if no topology refresh is due then
\State return previous topology state
\State end if
\State Compute pairwise squared node distances on GPU
\State Transfer distances to CPU and run Kruskal to obtain MST edges E\_t
\State Build adjacency from E\_t
\State Compute all-pairs graph hop distances g\_t with chunked GPU Floyd-Warshall
\State Deduplicate active radii and rebuild/update cached influence maps
\State Commit E\_t, g\_t, and cache state
...
\State Retain surviving RNG edges E\_t and build adjacency
\State Compute all-pairs graph hop distances g\_t with chunked GPU Floyd-Warshall
\State Deduplicate active radii and rebuild/update cached influence maps
\State Commit E\_t, g\_t, and cache state
\State return topology state
